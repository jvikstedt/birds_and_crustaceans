...
use bevy::{
    prelude::*,
    reflect::{serde::ReflectSerializer, Reflect, ReflectRef, TypeRegistry, TypeRegistryInternal},
    utils::HashMap,
};
use std::fmt::Debug;

use super::reflect_resource::ReflectResource;
use super::Rollback;
...
impl WorldSnapshot {
            ...
            for component_id in archetype.components() {
                ...
                if let Some(reflect_component) = reflect_component {
                        ...
                        .enumerate()
                    {
                        ...
                        {
                            if component.type_name() == "client::component::checksum::Checksum" {
                                if let ReflectRef::Struct(value) = component.reflect_ref() {
                                    let val = value.get_field::<f64>("value").unwrap();
                                    snapshot.checksum += val;
                                }
                            }
        ...
        for rollback_entity in self.entities.iter() {
            ...
            for registration in type_registry.iter() {
                ...
                let reflect_component = registration
                    ...
                    .expect("Unregistered Type in Rollback Type Registry");
    ...
    pub fn serialize(&self, type_registry: &TypeRegistryInternal) -> String {
        let mut str = String::new();

        for e in self.entities.iter() {
            str.push_str(
                format!(
                    "rollback_id: {}, components: {}, entity: {:?}",
                    e.rollback_id,
                    e.components.len(),
                    e.entity,
                )
            ...
            for c in e.components.iter() {
                let serializer = ReflectSerializer::new(&**c, type_registry);

                if let Ok(res) = serde_json::to_string(&serializer) {
